...
//-----------------------------------------------------------------------
/**
 * @class EC_CompanyForm
 * @fullname Eco Citoyen Management
 * @package EC_CompanyForm
 * @category Core
 * @filesource assets/plugins/Entreprise/EC_CompanyForm.php
 * @version 0.0.1
 * @created 2020-10-07
...
//--------------------------------------
if( ! class_exists( 'EC_CompanyForm' ) ):
	//----------------------------------
	class EC_CompanyForm
	{
		//------------------------------
		...
		//---
		const K_SLUG = 'ecCompanyForm';
		const K_PREFIX = 'ecCompanyForm-';
		const K_VERS = '0.0.1';
		...
		static function afterSubmission_fn($wpcf7)
		{
			...
			if ($submission) :
				...
				if ( empty( $postedData) ):
					...
					if( $thePost ):
						$ID = $dbFields['POST_ID'];
						// CHECK META FOEM ID
						$refDatas = get_post_meta($ID, EC_CompanyMBoxes::getOptionNameMB_fn(), TRUE);
			...
			///////////////////////////////////////////
			if( method_exists( 'EC_CompanyMsg', 'insertMsg_fn' ) ):
				//
				EC_CompanyMsg::insertMsg_fn( $dbFields );
				//
			endif;
			//-------------------------------------------------
			return $wpcf7;
	...
	//-------------------------------------------------------------------
	if( ! function_exists( 'ecCompanyForm_fn' ) ):
		function ecCompanyForm_fn() 
		{
			return EC_CompanyForm::getInstance_fn();
		};
	endif;
	//-------------------------------------------------------------------
	if( ! isset( $GLOBALS[ 'EC_CompanyForm' ] ) ):
		$GLOBALS[ 'EC_CompanyForm' ] = ecCompanyForm_fn();
	endif;
	//-------------------------------------------------------------------
endif;
//-----------------------------------------------------------------------