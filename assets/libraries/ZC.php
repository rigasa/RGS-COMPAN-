  	...
  	public function setSeriesData() {
        ...
        if ($numArgs == 1 && is_array(func_get_arg(0))) {
            for ($j = 0; $j < count(func_get_arg(0)); $j++) {
				$label = 'series['.$j.'].values';
				$value = func_get_arg(0);
				$this->setConfig( $label, $value[$j]);
            }
        }
        else if ($numArgs == 2) {
			
			$label = 'series['.func_get_arg(0).'].values';
			$value = func_get_arg(1);
            $this->setConfig($label, $value);
        }
        else {
    ...
    public function setSeriesText() {
        ...
        if ($numArgs == 1 && is_array(func_get_arg(0))) {
            for($i = 0; $i < count(func_get_arg(0)); $i++) {
				$label = 'series['.$i.'].text';
				$value = func_get_arg(0);
                $this->setConfig('series['.$i.'].text', $value[$i]);
                //$this->config['series'][$i]['text'] = func_get_arg(0)[$i];
            }
        }
        else if ($numArgs == 2) {
			$label = 'series['.func_get_arg(0).'].text';
			$value = func_get_arg(1);
            $this->setConfig($label, $value);
