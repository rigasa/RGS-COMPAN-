	...
	class RGS_CompanySettings
	{
		...
		static function getDefaultOptions_fn()
		{
			...
			foreach($formArchitect as $ftId => $formThemeArr):
				...
				$settings['colorSection_' . $ftId ] = '#' .array_rand( $colors, 1 );
		...
		static function getOption_fn($optName = '')
		{
			...
			if( empty( $optName ) ): $optName = self::getOptionName_fn(); endif;
			//
			$defOptions = (array) self::getDefaultOptions_fn();
			//$rgsOptions = (array) get_option( $optName, $defOptions );
			$rgsOptions = (array) get_option( $optName );
			//------------------
			//if( count( $rgsOptions ) == 0 ):
			//$rgsOptions = self::parseArgs_fn( $rgsOptions, $defOptions );
			//endif;
			//------------------
			return $rgsOptions;
			//------------------
		...
		public function initSettings_fn() 
		{
			...
			// Register a new field in the "wporg_sectionChart" section, inside the "pageID" page.
			add_settings_field(
				'themeChoice', 
		...
		function fieldSelectThemeCallback_fn( $args ) 
		{
			...
			<select
					...
					name="<?php echo $rgsOptionID; ?>[<?php echo esc_attr( $labelFor ); ?>]">
